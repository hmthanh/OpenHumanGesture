...
class MocapParameterizer(BaseEstimator, TransformerMixin):
    ...
    #         pos_cols = [c for c in euler_df.columns if ('position' in c and 'Nub' not in c)]
    #
    ...
    #
    #             rc = euler_df[[c for c in rot_cols if joint in c]]  # Get the columns that belong to this joint
    #             rot_order = track.skeleton[joint]['order']
    #             r1_col = '%s_%srotation' % (joint, rot_order[0])
    #             r2_col = '%s_%srotation' % (joint, rot_order[1])
    #             r3_col = '%s_%srotation' % (joint, rot_order[2])
    #
    #             pos_df.drop([r1_col, r2_col, r3_col], axis=1, inplace=True)
    #             euler_values = [[f[1][r1_col], f[1][r2_col], f[1][r3_col]] for f in rc.iterrows()]
    #
    #
    #
    #
    ...
    #
    #
    #             # pc = euler_df[[c for c in rot_cols if joint in c]]  # Get the columns that belong to this joint
    #             # r1_col = '%s_Xposition' % (joint)
    #             # r2_col = '%s_Yposition' % (joint)
    #             # r3_col = '%s_Zposition' % (joint)
    #             # pos_df.drop([r1_col, r2_col, r3_col], axis=1, inplace=True)
    #             # pos_values = [[f[1][r1_col], f[1][r2_col], f[1][r3_col]] for f in pc.iterrows()]
    #
    #
    #
    #
    #             # Convert the eulers to rotation matrices
    #             rotmats = R.from_euler(rot_order.lower(), euler_values, degrees=True).inv()
    #             tree_data[joint]=[
    ...
    #         tree_data = {}
    #
    #         # for joint in track.traverse():        # modify
    #         for joint in joints:
    #             parent = track.skeleton[joint]['parent']
    #             rot_order = track.skeleton[joint]['order']
    #
    ...
    def _to_pos(self, X):
        ...
        for track in X:
            ...
            # List the columns that contain rotation channels
            rots = [c for c in euler_df.columns if ('rotation' in c)]
            pos_cols = [c for c in euler_df.columns if ('position' in c)]
            ...
            for joint in joints:
                ...
                if r.shape[1] < 3:
                    ...
                    r3_col = '%s_%srotation' % (joint, rot_order[2])
                    pos_df.drop([r1_col, r2_col, r3_col], axis=1, inplace=True)
                    euler_values = [[f[1][r1_col], f[1][r2_col], f[1][r3_col]] for f in r.iterrows()]
    ...
    def _pos_to_euler(self, X):
        ...
        for track in X:
            ...
            # List the columns that contain rotation channels
            pos_params = [c for c in pos_df.columns if
                          (any(p in c for p in ['position']) and 'Nub' not in c)]

            # List the joints that are not end sites, i.e., have channels
            joints = (joint for joint in track.skeleton if 'Nub' not in joint)

            for joint in joints:
                pc = pos_df[[c for c in pos_params if joint in c]]  # Get the columns that belong to this joint
...
class ConstantsRemover(BaseEstimator, TransformerMixin):
    ...
    def __init__(self, eps=0, mode='rotation'):  # 1e-6
        self.eps = eps
        self.mode = mode
    ...
    def fit(self, X, y=None):
        ...
        if self.mode == 'rotation':
            self.const_dims_ = ['Hips_Xposition', 'Hips_Yposition', 'Hips_Zposition',
